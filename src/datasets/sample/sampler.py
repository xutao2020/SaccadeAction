...
class Sampler(data.Dataset):
    def __getitem__(self, id):
        ...
        cor_att = np.zeros((4, output_h, output_w), dtype=np.float32)
        # cor_att = np.zeros((K*4, output_h, output_w), dtype=np.float32)
        ...
        for ilabel in gt_bbox:
            for itube in range(len(gt_bbox[ilabel])):
                ...
                key_h, key_w = (gt_bbox[ilabel][itube][key, 3] - gt_bbox[ilabel][itube][key, 1],
                                gt_bbox[ilabel][itube][key, 2] - gt_bbox[ilabel][itube][key, 0])
                ...
                center = np.array([(gt_bbox[ilabel][itube][key, 0] + gt_bbox[ilabel][itube][key, 2]) / 2,
                                   (gt_bbox[ilabel][itube][key, 1] + gt_bbox[ilabel][itube][key, 3]) / 2], dtype=np.float32)
                ...
                idx = 0
                for y in (gt_bbox[ilabel][itube][key, 1], gt_bbox[ilabel][itube][key, 3]):
                    for x in (gt_bbox[ilabel][itube][key, 0], gt_bbox[ilabel][itube][key, 2]):
                        cor = np.array((x, y))
                        cor_int = cor.astype(np.int32)
                        draw_umich_gaussian(cor_att[idx], cor_int, int(radius + 0.5))
                        idx += 1

                for i in range(K):
                    center_all = np.array([(gt_bbox[ilabel][itube][i, 0] + gt_bbox[ilabel][itube][i, 2]) / 2,
                                           (gt_bbox[ilabel][itube][i, 1] + gt_bbox[ilabel][itube][i, 3]) / 2], dtype=np.float32)
                    center_all_int = center_all.astype(np.int32)
                    # wh is ground truth bbox's height and width in i_th frame
                    wh[num_objs, i * 2: i * 2 + 2] = (1. * (gt_bbox[ilabel][itube][i, 2] - gt_bbox[ilabel][itube][i, 0]),
                                                      1. * (gt_bbox[ilabel][itube][i, 3] - gt_bbox[ilabel][itube][i, 1]))
                    # mov is ground truth movement from i_th frame to key frame
                    mov[num_objs, i * 2: i * 2 + 2] = ((gt_bbox[ilabel][itube][i, 0] + gt_bbox[ilabel][itube][i, 2]) / 2 - center_int[0],
                                                       (gt_bbox[ilabel][itube][i, 1] + gt_bbox[ilabel][itube][i, 3]) / 2 - center_int[1])
                    ...
                    # idx = 0
                    # for y in (gt_bbox[ilabel][itube][i, 1], gt_bbox[ilabel][itube][i, 3]):
                    #     for x in (gt_bbox[ilabel][itube][i, 0], gt_bbox[ilabel][itube][i, 2]):
                    #         cor = np.array((x, y))
                    #         cor_int = cor.astype(np.int32)
                    #         draw_umich_gaussian(cor_att[idx+i*4], cor_int, int(radius + 0.5))
                    #         idx += 1
                ...
                mask[num_objs] = 1
                num_objs = num_objs + 1
        result = {'input': data, 'hm': hm, 'mov': mov, 'wh': wh, 'mask': mask, 'index': index, 'index_all': index_all, 'cor_att':cor_att}
