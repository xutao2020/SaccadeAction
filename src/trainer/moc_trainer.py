...
class MOCTrainLoss(torch.nn.Module):
    ...
    def forward(self, output, batch):
        ...
        output['cor_att'] = torch.clamp(output['cor_att'].sigmoid_(), min=1e-4, max=1 - 1e-4)

        hm_loss = self.crit_hm(output['hm'], batch['hm'])

        cor_att_loss = self.crit_hm(output['cor_att'], batch['cor_att']) / 4

        mov_loss = self.crit_mov(output['mov'], batch['mask'], batch['index'], batch['mov'])

        # wh_loss = self.crit_wh(output['wh'], batch['mask'], batch['index'], batch['wh'], index_all=batch['index_all'])

        final_wh_loss = self.crit_wh(output['final_wh'], batch['mask'], batch['index'], batch['wh'],
                                     index_all=batch['index_all'])

        loss = (opt.hm_weight * hm_loss + opt.mov_weight * mov_loss + opt.final_wh_weight * final_wh_loss +
        ...
        cor_att_loss = cor_att_loss.unsqueeze(0)
        final_wh_loss = final_wh_loss.unsqueeze(0)

        loss_stats = {'loss': loss, 'loss_hm': hm_loss, 'loss_mov': mov_loss, 'loss_final_wh': final_wh_loss,
                      'loss_cor_att': cor_att_loss}
...
class MOCTrainer(object):
    def __init__(self, opt, model, optimizer=None):
        ...
        self.loss_stats = ['loss', 'loss_hm', 'loss_mov', 'loss_final_wh', 'loss_cor_att']
    ...
    def run_epoch(self, phase, epoch, data_loader, writer):
        ...
        for iter, batch in enumerate(data_loader):
            ...
            Bar.suffix = '{phase}: [{0}][{1}/{2}]|Tot: {total:} |ETA: {eta:} |Lr: {lr:}'.format(
                ...
                total=bar.elapsed_td, eta=bar.eta_td, lr=lr)
