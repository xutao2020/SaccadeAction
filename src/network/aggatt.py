...
class AggAtt(nn.Module):
    ...
    def forward(self, input, wh):
        ...
        if self.mode == 'v1':
            mask = torch.cat([mask[:,0:1], po, mask[:,1:2],po,mask[:,2:3],po,mask[:,3:4],po,mask[:,4:5]],1)
            offset = torch.cat([-h / 2, -w / 2, po, po, -h / 2, w / 2, po, po, offset[:, :1], offset[:, 1:], po, po, \
            ...
            offset = torch.cat([-h / 2, -w / 2, -h / 2,po, -h / 2, w / 2, po, -w / 2, offset[:,:1], offset[:,1:], po, w / 2, \
